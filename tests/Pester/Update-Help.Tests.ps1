...
function RunUpdateHelpTests
{
    ...
    foreach ($moduleName in $modulesInBox)
    {
        It "Validate Update-Help for module '$moduleName'" {
            ...
            Get-ChildItem $testCases[$moduleName].HelpInstallationPath -Include @("about_*.txt","*help.xml") -Recurse -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
